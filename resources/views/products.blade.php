...
<div class="bg-stone-50 py-16 sm:py-24">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            <span class="text-rose-500 font-semibold tracking-wider uppercase text-sm">Etalase Kami</span>
            <h1 class="mt-2 text-4xl font-serif font-bold text-gray-900 sm:text-5xl">Produk Kecantikan</h1>
            <p class="mt-4 max-w-2xl text-lg text-gray-600 mx-auto">Produk perawatan rambut dan kecantikan premium untuk rutinitas perawatan di rumah Anda.</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            @forelse($products as $product)
                <div class="bg-white overflow-hidden shadow-sm hover:shadow-md transition duration-300 rounded-2xl border border-gray-100 flex flex-col group">
                    ...
                    @else
                        <div class="w-full h-64 bg-rose-50 flex items-center justify-center text-rose-300">
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        <h3 class="text-lg font-serif font-bold text-gray-900 mb-2">{{ $product->name }}</h3>
                        <p class="text-gray-600 text-sm flex-grow leading-relaxed">{{ Str::limit($product->description, 80) }}</p>
                        <div class="mt-4 pt-4 border-t border-gray-100 flex flex-col gap-2">
                            <span class="text-rose-600 font-bold text-xl">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            @empty

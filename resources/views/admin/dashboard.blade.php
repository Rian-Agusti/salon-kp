...
@section('header', 'Ringkasan Dasbor')
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-3xl font-serif font-bold text-gray-900">{{ $totalCustomers }}</p>
            </div>
            <div class="p-4 bg-rose-50 rounded-xl border border-rose-100 text-rose-500">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-3xl font-serif font-bold text-gray-900">{{ $totalReservations }}</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-3xl font-serif font-bold text-gray-900">{{ $pendingReservations }}</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-3xl font-serif font-bold text-gray-900">{{ $todaysReservations }}</p>
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
            <h3 class="text-lg font-serif font-bold text-gray-900">Layanan Terpopuler</h3>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-white">
                        <tr>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nama Layanan</th>
                            <th scope="col" class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Total Pesanan</th>
                        </tr>
                    </thead>
                        ...
                        @foreach($popularServices as $stat)
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 flex items-center gap-3">
                                    <span class="w-6 h-6 rounded-full bg-rose-50 text-rose-500 flex items-center justify-center text-xs border border-rose-100">{{ $loop->iteration }}</span>
                                    {{ $stat->service_name }}
                                </td>

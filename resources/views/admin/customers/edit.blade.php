...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 lg:w-2/3 xl:w-1/2">
    <form action="{{ route('admin.customers.update', $customer) }}" method="POST">
        ...
        <div class="space-y-4">
            ...
            <div id="password_fields" class="{{ old('type', $customer->type) === 'offline' ? 'hidden' : '' }} space-y-4">
                <!-- New Password -->
                <div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function togglePasswordFields() {
            if (typeSelect.value === 'offline') {
                ...
                passwordInputs.forEach(input => input.value = ''); // Clear password inputs when hiding
            } else {
                passwordFields.classList.remove('hidden');
            }
        }

        typeSelect.addEventListener('change', togglePasswordFields);

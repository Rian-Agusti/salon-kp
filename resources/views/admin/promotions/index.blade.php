        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Promotion
                        </th>
                        <th scope="col"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-16">
                                        @if($promotion->image)
                                            <img class="h-10 w-16 rounded object-cover" src="{{ Storage::url($promotion->image) }}"
                                                alt="">
                                        @else
                                            <div
                                                class="h-10 w-16 rounded bg-stone-200 flex items-center justify-center text-stone-400">
                                                Img
                                            </div>
                                        @endif
                                    </div>
    ...
    $isExpired = \Carbon\Carbon::parse($promotion->end_date)->isPast();
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                <form action="{{ route('admin.promotions.destroy', $promotion) }}" method="POST"
                                    class="inline-block" onsubmit="return confirm('Anda yakin ingin menghapus promosi ini?');">
                                    @csrf
                                    @method('DELETE')

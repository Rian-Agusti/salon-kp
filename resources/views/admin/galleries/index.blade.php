                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex-shrink-0 h-16 w-24">
                                    <img class="h-16 w-24 rounded object-cover" src="{{ Storage::url($gallery->image) }}"
                                        alt="{{ $gallery->title }}">
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-stone-100 text-stone-800 uppercase">
                                    {{ $gallery->category }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                <form action="{{ route('admin.galleries.destroy', $gallery) }}" method="POST"
                                    class="inline-block" onsubmit="return confirm('Anda yakin ingin menghapus galeri ini?');">
                                    @csrf
                                    @method('DELETE')

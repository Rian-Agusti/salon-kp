...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <h3 class="text-xl font-serif font-bold text-salon-text">Daftar Layanan</h3>
    <a href="{{ route('admin.services.create') }}" class="inline-flex justify-center items-center px-4 py-2.5 sm:py-2 bg-salon-gold border border-transparent rounded-lg font-bold text-sm text-white hover:bg-salon-goldHover transition w-full sm:w-auto">
...
<div class="bg-white shadow-sm border border-salon-beige rounded-2xl overflow-hidden">
                ...
                @forelse($services as $service)
                    <tr class="hover:bg-gray-50/50 transition">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-12 w-12">
                                    @if($service->image)
                                        <img class="h-12 w-12 rounded-xl object-cover border border-salon-beige" src="{{ Storage::url($service->image) }}" alt="">
                                    @else
                                        <div class="h-12 w-12 rounded-xl bg-gray-50 border border-salon-beige flex items-center justify-center text-gray-400">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                        </div>
                                    @endif
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-bold text-salon-text">{{ $service->name }}</div>
                                    <div class="text-sm text-gray-500">{{ Str::limit($service->description, 30) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-salon-gold">
                            Rp {{ number_format($service->price, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-salon-textLight">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @else
                                <span class="px-3 py-1 inline-flex text-xs font-bold rounded-full bg-gray-100 text-salon-textLight border border-gray-200">Nonaktif</span>
                            @endif
                        </td>
    ...
    @if($services->hasPages())
        <div class="bg-gray-50/50 px-6 py-4 border-t border-salon-beige">
            {{ $services->links() }}
        </div>
    @endif
</div>

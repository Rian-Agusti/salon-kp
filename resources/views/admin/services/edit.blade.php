...
@section('header', 'Edit Service: ' . $service->name)
    ...
    <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', (int)$service->price) }}" min="0" step="1" required
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-salon-gold focus:ring focus:ring-salon-beige focus:ring-opacity-50">
            ...
            <div>
                ...
                @if($service->image)
                    <div class="mb-2">
                        <img src="{{ Storage::url($service->image) }}" alt="Current Image" class="h-20 w-20 object-cover rounded border">
                    </div>
                @endif
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="4"
                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-salon-gold focus:ring focus:ring-salon-beige focus:ring-opacity-50">{{ old('description', $service->description) }}</textarea>
        ...
        <div class="flex justify-end pt-4 border-t border-gray-200">
            <a href="{{ route('admin.services.index') }}" class="bg-white border border-gray-300 text-gray-700 font-medium py-2 px-4 rounded shadow-sm hover:bg-gray-50 mr-3">Cancel</a>
            <button type="submit" class="bg-salon-gold hover:bg-salon-goldHover text-white font-medium py-2 px-4 rounded shadow-sm">Update Service</button>
        </div>
    </form>
</div>
@endsection

...
<div class="bg-stone-50 py-16 sm:py-24">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            <span class="text-rose-500 font-semibold tracking-wider uppercase text-sm">Katalog Kami</span>
            <h1 class="mt-2 text-4xl font-serif font-bold text-gray-900 sm:text-5xl">Layanan Perawatan</h1>
            <p class="mt-4 max-w-2xl text-lg text-gray-600 mx-auto">Temukan berbagai layanan perawatan rambut dan kecantikan premium yang dirancang khusus untuk Anda.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($services as $service)
                <div class="bg-white overflow-hidden shadow-sm hover:shadow-md transition duration-300 rounded-2xl border border-gray-100 flex flex-col group">
                    @if($service->image)
                    ...
                    @else
                        <div class="h-56 w-full bg-rose-50 flex items-center justify-center text-rose-300">
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        <div class="flex justify-between items-start gap-4 mb-4">
                            ...
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold bg-rose-50 text-rose-600 border border-rose-100">
                                Rp {{ number_format($service->price, 0, ',', '.') }}
                            </span>
                        </div>
                        <p class="text-sm text-gray-600 flex-1 leading-relaxed mb-6">{{ $service->description }}</p>
                        <a href="{{ route('customer.reservations.create', ['service_id' => $service->id]) }}" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-rose-500 hover:bg-rose-600 transition duration-300 {{ !$service->is_available ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' }}">
